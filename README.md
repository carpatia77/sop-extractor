...
| **Architecture reverse-engineering audit** ("Blackhat Mode") | `scripts/validate_architecture_audit.py` | Opt-in layer that reconstructs a demonstrated system's backend from its observable frontend, in a separate `<system>_architecture.md` where every line is sealed `[OBSERVED src/ts]` or `[INFERRED ← obs…]`. Four gates: **Seal**, **Grounding** (every inference cites real observations; persona-blind), **Non-Contamination** (no inference leaks into `SKILL.md`), **Intent** (human authorisation + analyst lens recorded). See [`docs/ARCHITECTURE_AUDIT.md`](docs/ARCHITECTURE_AUDIT.md). |
