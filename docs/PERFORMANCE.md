...
A before/after of the structural abstraction change (separating procedural execution (SOPs) from foundational principles):

| Metric | Flat Extraction | Separated Topology (`first_principles.md` + `sops.md`) |
|----------|---------:|---------:|
| Cognitive Load on LLM | High (interleaved theory and steps) | Low (isolated domains) |
| SOP-backed decisions | Mixed into general text | Extracted to dedicated `sops.md` |
| Token Budget per domain | ~2,000 (shared) | up to ~4,000 (SOPs) / ~3,000 (Principles) |

By explicitly separating **Load-bearing First Principles** from **Executable SOPs**, the skill allows the agent to reason about a problem conceptually *before* applying rigid procedural rules. The token budgets were intentionally expanded for these files based on real-world logs from dense procedural books, ensuring complex rulesets aren't truncated by arbitrary limits.
