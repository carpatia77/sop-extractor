...
def generate_colab_notebook(urls: list[str], model: str = "base") -> Path:
    ...
    cells.append(_create_notebook_cell("code", [
        ...
        "        if not audio:\n",
        "            print('  ❌ Erro no download - áudio não encontrado')\n",
        "            return None\n",
        "        print(f'  ✓ Áudio baixado: {audio.name} ({audio.stat().st_size / 1024 / 1024:.1f} MB)')\n",
        "        \n",
        "        # Transcrever\n",
        ...
        "        'segments': len(segs),\n",
        "        'gpu_time': elapsed,\n",
        "        'speedup': f'{info.duration/elapsed:.0f}x' if elapsed > 0 else 'N/A',\n",
        "    }\n",
        "    (out / 'metadata.json').write_text(json.dumps(meta, indent=2), encoding='utf-8')\n",
        "    \n",
        "    # Verificar arquivos salvos\n",
        "    saved_files = list(out.glob('*'))\n",
